...
switch name
    ...
    case 'four_branches_function_2'
        num_vari=2;   mu=[0,0];sigma=[1,1];design_space=[mu-5*sigma;mu+5*sigma];stopping_thresholds=[0.03 0.02 0.01];
    case 'cubic_function'
        num_vari=2;   mu=[10,9.9];sigma=[5,5];design_space=[mu-5*sigma;mu+5*sigma];stopping_thresholds=[0.03 0.02 0.01];
    case 'Circular_Pipe_function'
        num_vari=2;   mu=[301.079,0.503];sigma=[14.78,0.049];design_space=[mu-5*sigma;mu+5*sigma];stopping_thresholds=[0.03 0.02 0.01];
    case 'Hyper_sphere_bound_function'
        num_vari=2;   mu=[0.5,0.5];sigma=[0.2,0.2];design_space=[mu-5*sigma;mu+5*sigma];stopping_thresholds=[0.03 0.02 0.01];
    case 'Cantilever_beam_function'
        num_vari=2;   mu=[1000,250];sigma=[200,37.5];design_space=[mu-5*sigma;mu+5*sigma];stopping_thresholds=[0.03 0.02 0.01];
    ...
    case 'nonlinear_oscillator_function'
      ...
      case 'nonlinear_oscillator_function2'
        num_vari=6;   mu=[1,1,0.1,0.5,1,1];sigma=[0.05,0.1,0.01,0.05,0.2,0.2];design_space=[mu-5*sigma;mu+5*sigma];  stopping_thresholds=[0.05 0.03 0.01];
    ...
    case'Vehicle_side_impact_function'
           num_vari=7;   mu=[1.38 1.38 1.38 1.38 0.3 0 0];sigma=[0.3 0.3 0.3 0.3 0.06 10 10];design_space=[mu-5*sigma;mu+5*sigma];  
           stopping_thresholds=[0.03 0.02 0.01];
       case 'roof_truss'
          num_vari=6;  
